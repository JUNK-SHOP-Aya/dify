const translation = {
  ...
  stepTwo: {
    ...
    removeStopwords: '去除停用词，例如 “了”，“的”，“啊” 等',
    preview: '确认并预览',
    reset: '重置',
    indexMode: '索引方式',
    qualified: '高质量',
    recommend: '推荐',
    qualifiedTip: '调用 LLM 的嵌入接口进行处理，以在用户查询时提供更高的准确度，但是耗时长并且消耗 Token',
    warning: '请先完成模型供应商的 API KEY 设置。',
    click: '前往设置',
    economical: '经济',
    economicalTip: '使用离线的向量引擎、关键词索引等方式，降低了准确度但无需消耗 Token',
    emstimateCost: '执行嵌入预估消耗Token数',
  ...
  stepThree: {
    ...
    navTo: '前往文档',
    sideTipTitle: '接下来做什么',
    sideTipContent: '当文档完成索引处理后，数据集即可集成至应用内作为上下文使用，你可以在提示词编排页找到上下文设置。你也可以创建成可独立使用的 LLM 索引插件发布。',
